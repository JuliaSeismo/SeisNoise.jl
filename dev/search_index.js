...
{
    "location": "types/#SeisNoise-Types-1",
    "page": "Types",
    "title": "SeisNoise Types",
    "category": "section",
    "text": "SeisNoise is designed around array-based cross-correlation. SeisNoise uses three custom data structures: RawData for storing windows ambient noise, FFTData for storing Fourier transforms of ambient noise, and CorrData for storing cross-correlations. SeisNoise types extend SeisIO\'s SeisChannel type for 2D-array ambient noise processing. SeisNoise\'s modular functions work on RawData, FFTData, and CorrData objects in-place through Julia\'s multiple-dispatch. Functions in SeisNoise that end in ! (e.g. onebit!(R)) by convention modify their arguments, while functions that do not end in ! (e.g. onebit(R)) allocate new arrays or objects.note: Note\nBy convention in Julia, array data is stored column-wise. Noise windows in   RawData objects, ffts in FFTData objects, and cross-corrleations in   CorrData objects are all stored column-wise. To access data one or more time windows, use column-based indexing, e.g. R[:,1], F[:,2], or C[:,3:5]."
},
